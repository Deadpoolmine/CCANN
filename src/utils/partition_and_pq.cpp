...
int generate_pq_pivots(const float *passed_train_data, size_t num_train, unsigned dim, unsigned num_centers,
  ...
  // `num_train` is the number of training vectors
  // So, centroid is the mean vector of all training vectors
  ...
  // now every training vector is subtracted by centroid
  for (uint64_t d = 0; d < dim; d++) {
    for (uint64_t p = 0; p < num_train; p++) {
  ...
  // low_val/high_val are the two possible chunk sizes,
  // let's regard them as the same
  ...
  // The following codes ensure that each chunk has at
  // least low_val dimensions, ensuring balanced chunks.
  // Process dimensions not inserted by previous loop
  for (uint32_t d = 0; d < dim; d++) {
    // if dimension already assigned to a bin, skip
  ...
  // rearrangement is the order of dimensions after chunking
  // e.g., bin_to_dims = [[0,3],[1,4],[2,5]] for 6-dim 3-chunks
  // e.g., rearrangement: [0,3,1,4,2,5]
  // e.g., chunk_offsets: [0,2,4]
  for (uint32_t b = 0; b < num_pq_chunks; b++) {
  ...
  for (size_t i = 0; i < num_pq_chunks; i++) {
    ...
    // for each chunk, we have cur_chunk_size dimensions
    // and we should have num_centers pivots in this chunk
    std::unique_ptr<float[]> cur_pivot_data = std::make_unique<float[]>(num_centers * cur_chunk_size);
    // cur_data is the training data in this chunk
    std::unique_ptr<float[]> cur_data = std::make_unique<float[]>(num_train * cur_chunk_size);
    // closest_center is the closest center for each training vector in this chunk
    // maybe ID?
...
#pragma omp parallel for schedule(static, 65536)
    // copy the training data in this chunk to cur_data
  ...
  // pivot file layout
  // The REARRANGEMENT is used to rearrange the dimensions of the base vectors
  // the chunk_offsets is used to identify the chunks in the rearranged vectors
  // The centroid is used to add back to the reconstructed vectors
  // The full_pivot_data is the code words for each chunk in the rearranged vectors
  // |METADATA|PIVOT_DATA(num_centers (the first chunk), ...dim)|CENTROID(dim)|REARRANGEMENT(dim)|CHUNK_OFFSETS|
  std::vector<size_t> cumul_bytes(5, 0);
  cumul_bytes[0] = METADATA_SIZE;
  cumul_bytes[1] = cumul_bytes[0] + ccann::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                                             (size_t) num_centers, dim, cumul_bytes[0]);
  cumul_bytes[2] = cumul_bytes[1] +
                   ccann::save_bin<float>(pq_pivots_path.c_str(), centroid.get(), (size_t) dim, 1, cumul_bytes[1]);
  cumul_bytes[3] = cumul_bytes[2] + ccann::save_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement.data(),
                                                                rearrangement.size(), 1, cumul_bytes[2]);
  cumul_bytes[4] = cumul_bytes[3] + ccann::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                                                chunk_offsets.size(), 1, cumul_bytes[3]);
  ccann::save_bin<_u64>(pq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);
...
int generate_pq_data_from_pivots(const std::string data_file, unsigned num_centers, unsigned num_pq_chunks,
  ...
  if (!file_exists(pq_pivots_path)) {
    ...
    ccann::load_bin<_u64>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);
    ...
    ccann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc, file_offset_data[0]);
    ...
    ccann::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc, file_offset_data[1]);
    ...
    ccann::load_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement, nr, nc, file_offset_data[2]);
    ...
    ccann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc, file_offset_data[3]);
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    ccann::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);
...
#pragma omp parallel for schedule(static, 8192)
    ...
    if (num_centers > 256) {
      ...
      // block_data example: [(0, 3), (1, 4), (2, 5)] for 6-dim 3-chunks
      // block_compressed_base example: [0, 1, 2]
      std::unique_ptr<uint8_t[]> pVec = std::make_unique<uint8_t[]>(cur_blk_size * num_pq_chunks);
      ccann::convert_types<uint32_t, uint8_t>(block_compressed_base.get(), pVec.get(), cur_blk_size, num_pq_chunks);
...
int estimate_cluster_sizes(const std::string data_file, float *pivots, const size_t num_centers, const size_t dim,
  ...
  ccann::get_bin_metadata(data_file, num_points, num_dim);
  size_t block_size = num_points <= BLOCK_SIZE ? num_points : BLOCK_SIZE;
...
int shard_data_into_clusters(const std::string data_file, float *pivots, const size_t num_centers, const size_t dim,
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    base_reader.read((char *) block_data_T.get(), sizeof(T) * (cur_blk_size * dim));
    ccann::convert_types<T, float>(block_data_T.get(), block_data_float.get(), cur_blk_size, dim);
...
int partition_with_ram_budget(const std::string data_file, const double sampling_rate, double ram_budget,
  ...
  while (!fit_in_ram) {
    ...
    for (auto &p : cluster_sizes) {
      double cur_shard_ram_estimate = ccann::estimate_ram_usage(p, train_dim, sizeof(T), graph_degree);
  ...
  ccann::save_bin<float>(output_file.c_str(), pivot_data, (size_t) num_parts, train_dim);
