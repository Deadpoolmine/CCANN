...
namespace ccann {
  ...
  template<typename T, typename TagT>
  bool SSDIndex<T, TagT>::fast_delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool, int &to_evict,
                                                        float tgt_nbr_dis) {
      ...
      for (int i = (int) pool.size() - 1; i >= 0; --i) {
        ...
        if (pool[i].distance > tgt_nbr_dis) {
          ...
          if (tgt_nbr_dis / pool[i].tgt_dis > cur_alpha) {
            // NOTE: this means the target neighbor is of low quality.
            //       We should evict it.
            to_evict = this->range;
            pruned = true;
            break;
  ...
  template<typename T, typename TagT>
  void SSDIndex<T, TagT>::slow_delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool,
  ...
  void SSDIndex<T, TagT>::delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool,
    ...
    //   cur_alpha /= 1.2f;
    // }
    this->fast_delta_prune_neighbors_pq(pool, to_evict, tgt_nbr_dis);
    ...
    // to_evict = pool.size() - 1;
    // finish();
    this->slow_delta_prune_neighbors_pq(pool, pruned_list, scratch, tgt_idx);
  }
  ...
  void SSDIndex<T, TagT>::prune_neighbors(const tsl::robin_map<uint32_t, T *> &coord_map, std::vector<Neighbor> &pool,
    ...
    std::vector<Neighbor> result;
    // range is maximum out-neighbors
