...
namespace ccann {
  ...
  template<>
  ccann::Distance<float> *get_distance_function(ccann::Metric m) {
    if (m == ccann::Metric::L2) {
      return new ccann::DistanceL2();  // compile-time dispatch
    } else if (m == ccann::Metric::COSINE) {
      return new ccann::DistanceCosineFloat();
    } else {
  ...
  template<>
  ccann::Distance<int8_t> *get_distance_function(ccann::Metric m) {
    if (m == ccann::Metric::L2) {
      return new ccann::DistanceL2Int8();
    } else if (m == ccann::Metric::COSINE) {
      return new ccann::DistanceCosineInt8();
    } else {
      LOG(ERROR) << "Only L2 and cosine metric supported as of now";
      crash();
      return nullptr;
  ...
  template<>
  ccann::Distance<uint8_t> *get_distance_function(ccann::Metric m) {
    if (m == ccann::Metric::L2) {
      return new ccann::DistanceL2UInt8();
    } else if (m == ccann::Metric::COSINE) {
      LOG(INFO) << "AVX/AVX2 distance function not defined for Uint8. Using slow version.";
      return new ccann::SlowDistanceCosineUInt8();
    } else {
      LOG(ERROR) << "Only L2 and Cosine metric supported as of now.";
      crash();
      return nullptr;
