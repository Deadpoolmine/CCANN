...
namespace ccann {
  ...
  void add_new_file_to_single_index(std::string index_file, std::string new_file) {
    ...
    _u64 nr, nc;
    ccann::load_bin<_u64>(index_file, metadata, nr, nc, 0);
    if (nc != 1) {
    ...
    ccann::save_bin<_u64>(index_file, new_meta.data(), new_meta.size(), 1, 0);
  }
  ...
  size_t calculate_num_pq_chunks(double final_index_ram_limit, size_t points_num, uint32_t dim) {
    // num_pq_chunks is the size of each vector after PQ compression
    // points_num is the number of points in the dataset
    // So that num_pq_chunks * points_num is the size of the compressed dataset in bytes

    // So that even PQ vectors might overflow the memory budget
    // which only reduce the memory by num_pq_chunks times
    ...
    for (size_t i = 0; i < num_queries; i++) {
      ...
      for (auto &v : res) {
        if (gt.find(v) != gt.end()) {
          ...
          if (v == 0) {
            // this is a workaround for missing entries in truth set
            cur_recall++;
          }
        }
      }
  ...
  template<typename T>
  int build_merged_vamana_index(std::string base_file, ccann::Metric _compareMetric, bool single_file_index,
    ...
    ccann::get_bin_metadata(base_file, base_num, base_dim);
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      std::unique_ptr<ccann::Index<T>> _pvamanaIndex = std::unique_ptr<ccann::Index<T>>(
          new ccann::Index<T>(_compareMetric, base_dim, base_num, false, single_file_index, tags_enabled));
      if (tags_enabled)
    ...
    for (int p = 0; p < num_parts; p++) {
      ...
      std::unique_ptr<ccann::Index<T>> _pvamanaIndex = std::unique_ptr<ccann::Index<T>>(
          new ccann::Index<T>(_compareMetric, shard_base_dim, shard_base_pts, false,
    ...
    ccann::merge_shards(merged_index_prefix + "_subshard-", "_mem.index", merged_index_prefix + "_subshard-",
  ...
  void create_disk_layout(const std::string &mem_index_file, const std::string &base_file, const std::string &tag_file,
    ...
    if (vamana_frozen_num > 0) {
      ...
      size_t nr, nc;
      ccann::load_bin<TagT>(tag_file, mem_index_tags, nr, nc, tags_offset);
      ...
      ccann::save_bin<TagT>(output_file + std::string(".tags"), mem_index_tags.get(), nr, nc);
    } else {
      if (tags_enabled) {
        std::unique_ptr<TagT[]> mem_index_tags;
        size_t nr, nc;
    ...
    ccann::save_bin<_u64>(output_file, output_file_meta.data(), output_file_meta.size(), 1, 0);
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath, const char *indexBuildParameters,
                        ccann::Metric _compareMetric, bool single_file_index, const char *tag_file) {
    ...
    if (_compareMetric == ccann::Metric::COSINE) {
      if (std::is_floating_point<T>::value) {
        ...
        _compareMetric = ccann::Metric::L2;
      } else {
    ...
    // get data vectors points and dim
    ccann::get_bin_metadata(normalized_file_path, points_num, dim);
    ...
    // get p_val fraction `vectors` of points_num from normalized_file_path
    ...
    // Build Indexes
    start = std::chrono::high_resolution_clock::now();
    // Build a memory index first
    ccann::build_merged_vamana_index<T>(normalized_file_path, _compareMetric, single_file_index, L, R, p_val,
    ...
    if (tag_file == nullptr) {
      ccann::create_disk_layout<T, TagT>(mem_index_path, normalized_file_path, "", pq_pivots_path,
                                           pq_compressed_vectors_path, single_file_index, disk_index_path);
    } else {
      std::string tag_filename = std::string(tag_file);
      ccann::create_disk_layout<T, TagT>(mem_index_path, normalized_file_path, tag_filename, pq_pivots_path,
                                           pq_compressed_vectors_path, single_file_index, disk_index_path);
    }
  ...
  bool build_disk_index_py(const char *dataPath, const char *indexFilePath, uint32_t R, uint32_t L, uint32_t M,
                           uint32_t num_threads, uint32_t PQ_bytes, ccann::Metric _compareMetric,
    ...
    LOG(INFO) << "Compressed data generated and written in: " << std::chrono::duration<double>(end - start).count()
              << "s.";
    start = std::chrono::high_resolution_clock::now();
    ccann::build_merged_vamana_index<T>(normalized_file_path, _compareMetric, single_file_index, L, R, p_val, M,
  ...
  template int build_merged_vamana_index<int8_t>(std::string base_file, ccann::Metric _compareMetric,
                                                 ...
                                                 const char *tag_file);
  template int build_merged_vamana_index<float>(std::string base_file, ccann::Metric _compareMetric,
                                                bool single_file_index, unsigned L, unsigned R, double sampling_rate,
                                                double ram_budget, std::string mem_index_path, std::string medoids_path,
                                                std::string centroids_file, const char *tag_file);
  template int build_merged_vamana_index<uint8_t>(std::string base_file, ccann::Metric _compareMetric,
                                                  bool single_file_index, unsigned L, unsigned R, double sampling_rate,
                                                  double ram_budget, std::string mem_index_path,
                                                  std::string medoids_path, std::string centroids_file,
                                                  const char *tag_file);
