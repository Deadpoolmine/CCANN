...
namespace ccann {
  ...
  Index<T, TagT>::Index(Metric m, const size_t dim, const size_t max_points, const bool dynamic_index,
    ...
    LOG(INFO) << "Getting distance function for metric: " << (m == ccann::Metric::COSINE ? "cosine" : "l2");
  ...
  void Index<T, TagT>::save(const char *filename) {
    ...
    if (!_save_as_one_file) {
      ...
      ccann::save_bin<_u64>(filename, cumul_bytes.data(), cumul_bytes.size(), 1, 0);
  ...
  size_t Index<T, TagT>::load_data(std::string filename, size_t offset) {
    ...
    size_t file_dim, file_num_points;
    ccann::get_bin_metadata(filename, file_num_points, file_dim, offset);
  ...
  void Index<T, TagT>::load(const char *filename) {
    ...
    if (!_save_as_one_file) {
      ...
      ccann::load_bin<_u64>(index_file, file_offset_data, nr, nc, 0);
  ...
  void Index<T, TagT>::load_from_disk_index(const std::string &filename) {
    ...
    char *buf;
    ccann::alloc_aligned((void **) &buf, kSectorsPerRead * kSectorLen, kSectorLen);
...
#pragma omp critical
   ...
   * in the graph.
   */
  // medoid node: the node with minimum sum of distances to all other nodes
  template<typename T, typename TagT>
      ...
      if (best_L_nodes[k].flag) {
        ...
        for (unsigned m = 0; m < des.size(); ++m) {
          ...
          if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
            ...
            if ((m + 1) < des.size()) {
              auto nextn = des[m + 1];
              ccann::prefetch_vector((const char *) _data + _aligned_dim * (size_t) nextn, sizeof(T) * _aligned_dim);
            }
...
#pragma omp parallel for schedule(dynamic, 65536)
  ...
  void Index<T, TagT>::build(const char *filename, const size_t num_points_to_load, Parameters &parameters,
    ...
    if (filename == nullptr) {
      ...
      ccann::get_bin_metadata(filename, file_num_points, file_dim);
  ...
  void Index<T, TagT>::build(const char *filename, const size_t num_points_to_load, Parameters &parameters,
      ...
      if (_enable_tags) {
        if (tag_filename == nullptr) {
          ...
          if (file_exists(tag_filename)) {
            ...
            size_t npts, ndim;
            ccann::load_bin(tag_filename, tag_data, npts, ndim);
    ...
    generate_frozen_point();
    link(parameters);  // Primary func for creating nsg graph (neighborhood sparse graph)
  ...
  uint32_t Index<T, TagT>::search_with_tags_fast(const T *node_coords, const unsigned Lsize, TagT *tags, float *dists) {
    ...
    for (uint32_t i = 0; i < Lsize; ++i) {
      // avoid insert _ep (dynamic point)
      auto id = best_L_nodes[i].id;
      if (_location_to_tag.find(id) != _location_to_tag.end()) {
        tags[i] = _location_to_tag[id];
  ...
  int Index<T, TagT>::generate_frozen_point() {
    ...
    // NOTE: frozen point is a copy of entry point
    // add frozen point to the end of data array
...
#pragma omp parallel for schedule(dynamic)
  ...
  // NOTE: migrate the old_location point to new_location point
  template<typename T, typename TagT>
