...
int search_disk_index(int argc, char **argv) {
  ...
  ccann::AbstractNeighbor<T> *nbr_handler = ccann::get_nbr_handler<T>(m, nbr_type);
  ccann::IndexBuildParameters idx_params;
  idx_params.max_nthreads = num_threads; // limit the number of allocated buffers.
  std::unique_ptr<ccann::SSDIndex<T>> _pFlashIndex(
      new ccann::SSDIndex<T>(m, reader, nbr_handler, true, &idx_params));
  ...
  auto ret = ccann::dsl::load_filter_from_json(
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float mean_latency =
        ccann::get_mean_stats(stats, query_num, [](const ccann::QueryStats &s) { return s.total_us; });
    float latency_99 =
        ccann::get_percentile_stats(stats, query_num, 0.99f, [](const ccann::QueryStats &s) { return s.total_us; });
    float mean_hops = ccann::get_mean_stats(stats, query_num, [](const ccann::QueryStats &s) { return s.n_hops; });
    float mean_ios = ccann::get_mean_stats(stats, query_num, [](const ccann::QueryStats &s) { return s.n_ios; });

    float filter_ratio[ccann::N_FILTER_TYPES] = {0};
    for (int i = 0; i < ccann::N_FILTER_TYPES; i++) {
      filter_ratio[i] =
          ccann::get_mean_stats(stats, query_num, [i](const ccann::QueryStats &s) { return s.n_filter[i]; });
    }

    struct FilterStats {
      double n_est_filter_reads[ccann::N_FILTER_TYPES] = {0};
      double n_est_filter_cmps[ccann::N_FILTER_TYPES] = {0};
      double n_filter_reads[ccann::N_FILTER_TYPES] = {0};
      double n_filter_accessed_vectors[ccann::N_FILTER_TYPES] = {0};
      double n_filter_false_positives[ccann::N_FILTER_TYPES] = {0};
    };
    FilterStats filter_stats;

    for (int i = 0; i < ccann::N_FILTER_TYPES; i++) {
      filter_stats.n_est_filter_reads[i] =
          ccann::get_mean_stats(stats, query_num * filter_ratio[i], query_num,
                                  [i](const ccann::QueryStats &s) { return s.n_est_filter_reads[i]; });
      filter_stats.n_filter_reads[i] =
          ccann::get_mean_stats(stats, query_num * filter_ratio[i], query_num,
                                  [i](const ccann::QueryStats &s) { return s.n_filter_reads[i]; });
      filter_stats.n_filter_accessed_vectors[i] =
          ccann::get_mean_stats(stats, query_num * filter_ratio[i], query_num,
                                  [i](const ccann::QueryStats &s) { return s.n_filter_accessed_vectors[i]; });
      filter_stats.n_filter_false_positives[i] =
          ccann::get_mean_stats(stats, query_num * filter_ratio[i], query_num,
                                  [i](const ccann::QueryStats &s) { return s.n_filter_false_positives[i]; });
      filter_stats.n_est_filter_cmps[i] =
          ccann::get_mean_stats(stats, query_num * filter_ratio[i], query_num,
                                  [i](const ccann::QueryStats &s) { return s.n_est_filter_cmps[i]; });
    }
    delete[] stats;
    ...
    if (output) {
      ...
      if (calc_recall_flag) {
        recall =
            ccann::calculate_recall((uint32_t) query_num, gt_ids, gt_dists, (uint32_t) gt_dim,
      ...
      std::cout << std::setw(4) << L << std::setw(4) << beamwidth << std::setw(10) << qps << std::setw(8)
                << mean_latency << std::setw(10) << query_num * filter_ratio[ccann::PRE_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_reads[ccann::PRE_FILTER] << std::setw(8)
                << filter_stats.n_filter_reads[ccann::PRE_FILTER] << std::setw(8)
                << filter_stats.n_filter_accessed_vectors[ccann::PRE_FILTER] << std::setw(6)
                << filter_stats.n_filter_false_positives[ccann::PRE_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_cmps[ccann::PRE_FILTER] << std::setw(10)
                << query_num * filter_ratio[ccann::IN_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_reads[ccann::IN_FILTER] << std::setw(8)
                << filter_stats.n_filter_reads[ccann::IN_FILTER] << std::setw(8)
                << filter_stats.n_filter_accessed_vectors[ccann::IN_FILTER] << std::setw(6)
                << filter_stats.n_filter_false_positives[ccann::IN_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_cmps[ccann::IN_FILTER] << std::setw(10)
                << query_num * filter_ratio[ccann::POST_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_reads[ccann::POST_FILTER] << std::setw(8)
                << filter_stats.n_filter_reads[ccann::POST_FILTER] << std::setw(8)
                << filter_stats.n_filter_accessed_vectors[ccann::POST_FILTER] << std::setw(6)
                << filter_stats.n_filter_false_positives[ccann::POST_FILTER] << std::setw(8)
                << filter_stats.n_est_filter_cmps[ccann::POST_FILTER] << std::setw(8) << mean_ios;
      if (calc_recall_flag) {
