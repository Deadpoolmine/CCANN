...
void exact_knn(const size_t dim, const size_t k,
               ...
               size_t npoints, float *points_in, size_t nqueries, float *queries_in, ccann::Metric metric,
               const FilterFn &filter) {
  ccann::Distance<float> *distance = ccann::get_distance_function<float>(metric);
...
inline void load_bin_as_float(const char *filename, float *&data, uint64_t &npts, uint64_t &ndims, int part_num) {
  ...
  ccann::alloc_aligned((void **) &data, npts * ndims * sizeof(float), ALIGNMENT);
...
template<typename T>
int aux_main(int argc, char **argv, ccann::Metric metric, const std::string &label_config_file) {
  ...
  if (metric == ccann::Metric::COSINE) {
    for (uint64_t i = 0; i < nqueries; i++) {
      ccann::normalize_data(query_data + i * dim, query_data + i * dim, dim);
    }
  }
  ...
  ccann::Selector *selector = nullptr;
  std::map<uint32_t, ccann::AttrIndex *> base_stores;
  std::vector<ccann::Attributes> query_attrs;

  if (!label_config_file.empty()) {
    std::cerr << "Loading label config from " << label_config_file << " with " << n_base_points << " base points"
              << std::endl;
    std::tie(selector, query_attrs, base_stores) = ccann::dsl::load_filter_from_json(label_config_file, n_base_points);
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    if (metric == ccann::Metric::COSINE) {
      for (uint64_t i = 0; i < npoints; i++) {
        ccann::normalize_data(base_data + i * dim, base_data + i * dim, dim);
      }
    }
  ...
  for (uint64_t i = 0; i < nqueries; i++) {
    ...
    for (uint64_t j = 0; j < k; j++) {
      if (j < valid_count) {
        ...
        if (metric == ccann::Metric::INNER_PRODUCT)
...
int main(int argc, char **argv) {
  ...
  ccann::Metric metric = ccann::get_metric(dist_fn);
