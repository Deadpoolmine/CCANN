...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    ...
    TRUTH_FILE=/home/data/deadpool/ANN/SIFT-SMALL/data/siftsmall_groundtruth.bin
    ...
    TRUTH_FILE=/home/data/deadpool/ANN/SIFT-100M/data/100M_gt.bin
    ...
    TRUTH_FILE=/home/data/deadpool/ANN/SIFT-1M/data/1M_gt.bin
    QUERY_BIN=/home/data/deadpool/ANN/SIFT-1M/data/1M_query.bbin
elif [[ "${WORKLOAD}" == "deep-100m" ]]; then
    INDEX_PATH=/home/data/deadpool/ANN/DEEP-100M/index/
    TRUTH_FILE=/home/data/deadpool/ANN/DEEP-100M/data/100M_gt.bin
    QUERY_BIN=/home/data/deadpool/ANN/DEEP-100M/data/100M_queries.fbin
elif [[ "${WORKLOAD}" == "bigann" ]]; then
    INDEX_PATH=/home/data/straho/indices/bigann/0_500m
    TRUTH_FILE=/home/data/deadpool/ANN/SIFT-500M/update_gt_100_1000000/gt_0.bin
    QUERY_BIN=/home/data/deadpool/ANN/SIFT-100M/data/bigann_query.bbin
fi
...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    ...
    INDEX_PREFIX=$INDEX_PATH/0_500m
    TYPE_PREFIX=uint8
elif [[ "${WORKLOAD}" == "bigann-1m" ]]; then
...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    ...
    rm $INDEX_PATH/0_500m_shadow_disk.index
    rm $INDEX_PATH/0_500m_shadow_journal.log
    rm $INDEX_PATH/0_500m_shadow_pq_compressed.bin
    rm $INDEX_PATH/0_500m_shadow_pq_pivots.bin
    rm $INDEX_PATH/0_500m_merge_journal*
elif [[ "${WORKLOAD}" == "deep-100m" ]]; then
    rm $INDEX_PATH/100M_shadow_disk.index
    rm $INDEX_PATH/100M_shadow_journal.log
    rm $INDEX_PATH/100M_shadow_pq_compressed.bin
...
if (( PERF == 1 )); then    
    ...
    sudo perf record -g -F 5000 -o $PERF_DATA -- \
        $UTILS_PATH/search "$TYPE_PREFIX" $L_disk $SEARCH_THREADS $SEARCH_MODE $INDEX_PREFIX $QUERY_BIN $TRUTH_FILE $RECALL $BEAM_WIDTH $SEARCH_BEAM_WIDTH $SEARCH_MEM_L $SEARCH_L
