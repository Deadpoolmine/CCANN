...
int main(int argc, char **argv) {
  if (argc < 11) {
    ...
    std::map<uint32_t, ccann::AttrIndex *> labels;
    ...
    for (int i = first_label_index; i < argc; i += 2) {
      ...
      if (label_type == "label_spmat") {
        ...
        auto val = new ccann::InvertedLabelAttrIndex(label_filename(index_prefix_path, key), 0);
        val->load_from_spmat(label_file);
        labels[key] = val;
      } else if (label_type == "range") {
        auto val = new ccann::SortedRangeAttrIndex(label_filename(index_prefix_path, key), 0);
        if (ccann::is_suffix(label_file, ".bin")) {
          LOG(INFO) << "Loading range bin " << key << " from " << label_file;
          val->load_from_bin(label_file);
        } else if (ccann::is_suffix(label_file, ".spmat")) {
    ...
    if (std::string(argv[2]) != "null") {
      auto attr_writer = new ccann::AttrIndexWriter(labels);

      std::string nbr_type = argv[11];
      if (std::string(argv[1]) == std::string("float")) {
        ccann::AbstractNeighbor<float> *nbr_handler = ccann::get_nbr_handler<float>(m, nbr_type);
        ccann::build_disk_index<float>(argv[2], argv[3], std::stoi(argv[4]), std::stoi(argv[6]), std::stoi(argv[8]),
        ...
        ccann::build_disk_index<int8_t>(argv[2], argv[3], std::stoi(argv[4]), std::stoi(argv[6]), std::stoi(argv[8]),
                                          std::stoi(argv[9]), std::stoi(argv[7]), m, nullptr, nbr_handler, attr_writer,
                                          std::stoi(argv[5]));
      } else if (std::string(argv[1]) == std::string("uint8")) {
        ccann::AbstractNeighbor<uint8_t> *nbr_handler = ccann::get_nbr_handler<uint8_t>(m, nbr_type);
        ccann::build_disk_index<uint8_t>(argv[2], argv[3], std::stoi(argv[4]), std::stoi(argv[6]), std::stoi(argv[8]),
