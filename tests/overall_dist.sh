...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    ...
    TRUTH_SET=/home/data/deadpool/ANN/SIFT-500M/update_gt_100_1000000/
    QUERY_BIN=/home/data/straho/data/bigann/bigann_query.bbin
    TRUTH_SET_OFFSET=0
fi

if (( ON_PM == 1 )); then
    INDEX_PATH0=/mnt/pmem0/index
    INDEX_PATH1=/mnt/pmem1/index
fi
...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    INDEX_PREFIX0=$INDEX_PATH0/siftsmall
    INDEX_PREFIX1=$INDEX_PATH1/siftsmall
    TYPE_PREFIX=uint8
elif [[ "${WORKLOAD}" == "bigann-100m" ]]; then
    INDEX_PREFIX0=$INDEX_PATH0/100M
    INDEX_PREFIX1=$INDEX_PATH1/100M
    TYPE_PREFIX=uint8
elif [[ "${WORKLOAD}" == "bigann" ]]; then
    INDEX_PREFIX0=$INDEX_PATH0/0_500m
    INDEX_PREFIX1=$INDEX_PATH1/0_500m
...
if (( PERF == 1 )); then    
    ...
    $UTILS_PATH/distribute_test_insert_search "$TYPE_PREFIX" "$DATA_BIN" $L_disk "$VECS_PER_STEP" "$NUM_STEPS" "$INSERT_THREADS" "$SEARCH_THREADS" "$SEARCH_MODE" 2 "$INDEX_PREFIX0" "$INDEX_PREFIX1" "$QUERY_BIN" "$TRUTH_SET" "$TRUTH_SET_OFFSET" $RECALL $BEAM_WIDTH $SEARCH_BEAM_WIDTH $SEARCH_MEM_L $INSERT_ONLY ${SEARCH_L[@]}
fi
