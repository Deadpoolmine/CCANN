...
void merge_kernel(ccann::DynamicSSDIndex<T, TagT> &sync_index) {
  // use either fixed number of threads or num search threads.
  sync_index.final_merge(8);
}

template<typename T, typename TagT>
float insertion_kernel(T *data_load, ccann::DynamicSSDIndex<T, TagT> &sync_index, std::vector<TagT> &insert_vec,
                       size_t dim, unsigned merge_interval, unsigned &counter) {
  ccann::Timer timer;
...
void update(const std::string &data_bin, const unsigned L_disk, int vecs_per_step, int num_steps,
  ...
  for (int i = 0; i < num_steps; i++) {
    ...
    // std::future<float> insert_future = std::async(std::launch::async, insertion_kernel<T, TagT>, data_load.data(),
    //                                               std::ref(sync_index), std::ref(insert_vec), dim, merge_interval, std::ref(counter));
    ...
    // total_time_secs = insert_future.get();

    total_time_secs +=
        insertion_kernel<T, TagT>(data_load.data(), sync_index, insert_vec, dim, merge_interval, counter);
...
int main(int argc, char **argv) {
  if (argc < 9) {
    LOG(INFO) << "Correct usage: " << argv[0] << " <type[int8/uint8/float]> <data_bin> <L_disk>"
              ...
              << " <index_prefix> "
              << " <#beam_width> <merge_interval> <search_mode>";
    exit(-1);
  }
  ...
  unsigned merge_interval = (unsigned) std::atoi(argv[arg_no++]);
