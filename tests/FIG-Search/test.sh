...
MEDIAS=(1)
CANDIDATE_LS=(10 15 20 25 30 35 40 50 60 80 120 200 400)

ANNS=("cc-ann" "ccann" "diskann")
...
for WORKLOAD in "${WORKLOADS[@]}"; do
    ...
    for media in "${MEDIAS[@]}"; do
        ...
        for ann in "${ANNS[@]}"; do        
            ...
            bash "$ABS_PATH"/../tools/setup-cc-ann.sh CCANN "$ann" 0
            ...
            for t in "${THREADS[@]}"; do
                for L in "${CANDIDATE_LS[@]}"; do
                    ...
                    OUTPUT=$(bash "$ABS_PATH"/../search.sh CCANN "$media" 0 "$t" "$WORKLOAD" "$search_mode" "$L" 2>&1)
