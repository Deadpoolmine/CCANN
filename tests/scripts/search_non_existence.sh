...
if (( PERF == 1 )); then    
    ...
    $UTILS_PATH/search_non_existence uint8 $DATA_BIN $L_disk $VECS_PER_STEP $NUM_STEPS $INSERT_THREADS $SEARCH_THREADS $SEARCH_MODE $INDEX_PREFIX $QUERY_BIN $TRUTH_SET $TRUTH_SET_OFFSET $RECALL $BEAM_WIDTH $SEARCH_BEAM_WIDTH $SEARCH_MEM_L $SEARCH_L
fi
