...
for WORKLOAD in "${WORKLOADS[@]}"; do
    ...
    for media in "${MEDIAS[@]}"; do
        ...
        for ann in "${ANNS[@]}"; do
            if [[ "${ann}" == *"odinann"* ]]; then
                bash "$ABS_PATH"/../tools/setup-cc-ann.sh CCANN "odinann" 0
            ...
            bash "$ABS_PATH"/../overall.sh CCANN "$media" 0 "$insert_t" "$search_t" $batch_size $n_batches "$WORKLOAD" "$search_mode" |& tee "$output_path"
