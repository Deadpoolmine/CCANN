...
#define _round_up(X, Y) ((((uint64_t) (X) / (Y)) + ((uint64_t) (X) % (Y) != 0)) * (Y))

#define ROUND_UP(X, Y) (X == 0 ? Y : _round_up(X, Y))
    ...
    if (rc != 0) {
      LOG(ERROR) << "Could not delete file: " << fileName
                 << " even though it exists. This might indicate a permissions issue. "
                    "If you see this message, please contact the ccann team.";
    }
    return rc;
  } else {
    return 0;
...
template<typename T>
ccann::Distance<T> *get_distance_function(ccann::Metric m);
