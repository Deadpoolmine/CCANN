  ...
  class DynamicSSDIndex {
    ...
    */
    DynamicSSDIndex(Parameters &parameters, const std::string disk_prefix_in, const std::string disk_prefix_out,
                    Distance<T> *dist, ccann::Metric disk_metric, int search_mode = BEAM_SEARCH,
                    bool use_mem_index = false, bool read_only = false,
                    int cpu_bound = std::thread::hardware_concurrency());
   ...
   private:
    ...
    void build_id2loc_mapping(std::string &index_file, std::string &id2loc_file);
   ...
   public:
    ...
    ccann::Metric _dist_metric;
    Distance<T> *_dist_comp;

    ccann::Parameters _paras_mem;
    ccann::Parameters _paras_disk;
