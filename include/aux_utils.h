...
namespace ccann {
  ...
  template<typename T>
  int build_merged_vamana_index(std::string base_file, ccann::Metric _compareMetric, bool single_index_file,
  ...
  bool build_disk_index(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                        ccann::Metric _compareMetric, bool single_file_index, const char *tag_file = nullptr);
  template<typename T, typename TagT = uint32_t>
