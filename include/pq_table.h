  ...
  class FixedChunkPQTable {
   ...
   public:
    ...
    void load_pq_pivots_new(std::basic_istream<char> &reader, size_t num_chunks, size_t offset) {
      ...
      ccann::load_bin_impl<_u64>(reader, file_offset_data_raw, nr, nc, offset);
      ...
      ccann::load_bin_impl<float>(reader, tables, nr, nc, file_offset_data[0] + offset);
      ...
      this->ndims = nc;
      ccann::load_bin_impl<float>(reader, centroid, nr, nc, file_offset_data[1] + offset);
      ...
      ccann::load_bin_impl<uint32_t>(reader, rearrangement, nr, nc, file_offset_data[2] + offset);
      if ((nr != this->ndims) || (nc != 1)) {
        LOG(ERROR) << "Rearrangement incorrect: row " << nr << ", col " << nc << " expecting " << this->ndims;
        crash();
      }

      ccann::load_bin_impl<uint32_t>(reader, chunk_offsets, nr, nc, file_offset_data[3] + offset);
    ...
    void post_load_pq_table() {
      ...
      ccann::alloc_aligned((void **) &tables_T, 256 * ndims * sizeof(float), 64);
    ...
    void populate_chunk_distances(const T *query_vec, float *dist_vec) {
      // 256 is the num_centers per chunk
      // n_chunks is the compressed dimension
    ...
    void compute_distances_alltoall(const _u8 *comp_src, const _u8 *comp_dsts, float *dists, const _u32 count) {
      ...
      //     dists[i] +=
      //         all_to_all_dists[(_u64) comp_src[c] * 256 * n_chunks + (_u64) comp_dsts[i * n_chunks + c] * n_chunks +
      //         c];
      //   }
      // }

      const _u64 chunks = n_chunks;  // local copy for speed
      for (_u64 c = 0; c < chunks; ++c) {
        const _u64 src_base = (_u64) comp_src[c] * 256 * chunks;
        for (_u64 i = 0; i < count; ++i) {
          const _u8 dst = comp_dsts[i * chunks + c];
          dists[i] += all_to_all_dists[src_base + (_u64) dst * chunks + c];
        }
      }
    }
