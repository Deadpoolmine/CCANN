...
namespace {
  ...
  // compute the distance of query to each point in `pq_ids`
  inline void pq_dist_lookup(const _u8 *pq_ids, const _u64 n_pts, const _u64 pq_nchunks, const float *pq_dists,
    ...
    // Explain
    // We have a query vector [(1, 2), (1, 3), (2, 4)]
    // We calculate pq_dists for this query vector
    // There is a code book
    // e.g., [ [(0, 1), (1, 2) ..., (256, 257)]  // chunk 0
    //         [(0, 1), (1, 2) ..., (256, 257)] // chunk 1
    //         [(0, 1), (1, 2) ..., (256, 257)] ]
    // The query vector can be represented as the closest center ids in each chunk
    // e.g., [1, 2, 3]
    // pq_dists is precomputed query <-> chunk center distances
    // e.g., [ [0, 1, ..., 255]  // chunk 0
    //         [0, 1, ..., 255] // chunk 1
    //         [0, 1, ..., 255] ]
    // pq_ids is compressed vectors
    // e.g., [0, 1, 2] => [(0, 1), (1, 2), (1, 3)] => dist = [pq_dists[0][0], pq_dists[1][1], pq_dists[2][2]]
    //       [1, 2, 3], npts = 2, pq_nchunks = 3, centers = 256
    // prefetch_chunk_dists(pq_dists);
    memset(dists_out, 0, n_pts * sizeof(float));
    for (_u64 chunk = 0; chunk < pq_nchunks; chunk++) {
      const float *chunk_dists = pq_dists + 256 * chunk;
      // chunk_dists[idx] means the distance of this chunk (of query) to center idx
      if (chunk < pq_nchunks - 1) {
        prefetch_chunk_dists(chunk_dists + 256);
      }
      // this calculates the distance of query to each point
#pragma GCC ivdep
...
#define READ_UNSIGNED(stream, val) stream.read((char *) &val, sizeof(unsigned))
  ...
  class SSDIndex {
   public:
#define N_JOURNAL 32
#define ANN_INIT_TIMING(t) timespec t;
#ifdef ANN_TIMING
#define ANN_START_TIMING(stat, t)      \
  {                                    \
    clock_gettime(CLOCK_REALTIME, &t); \
  }
#define ANN_END_TIMING(stat, t)                                                                            \
  {                                                                                                        \
    timespec end;                                                                                          \
    clock_gettime(CLOCK_REALTIME, &end);                                                                   \
    auto cur_cpuid = sched_getcpu();                                                                       \
    this->io_timing_stats[cur_cpuid]->stat += ((end.tv_sec - t.tv_sec) * 1e9 + (end.tv_nsec - t.tv_nsec)); \
  }
#define ANN_ADD_STAT(stat, delta)                    \
  {                                                  \
    auto cur_cpuid = sched_getcpu();                 \
    this->io_timing_stats[cur_cpuid]->stat += delta; \
  }
#else
#define ANN_START_TIMING(stat, t)
#define ANN_END_TIMING(stat, t)
...
#define SEARCH_BATCH_SIZE(nnbr) (nnbr / 2)
    ...
    v2::Journal<TagT> **journals;

    void *get_cur_journal_instance() {
      int cpu_id = sched_getcpu();
      int journal_id = cpu_id % N_JOURNAL;
      auto journal = this->journals[journal_id];

      if (journal == nullptr) {
        LOG(ERROR) << "Journal is null in checkpoint";
        return nullptr;
      }
      return journal;
    }
    ...
    inline uint64_t u_loc_offset(uint64_t loc) {
      ...
      auto sector_no = loc_sector_no(loc);
      auto sector_off = (nnodes_per_sector == 0 ? 0 : (loc % nnodes_per_sector) * max_node_len);
      return sector_no * SECTOR_LEN + sector_off;
    }
    ...
    void init_query_buf(QueryBuffer<T> &buf) {
      ...
      ccann::alloc_aligned((void **) &buf.aligned_pq_coord_scratch, MAX_N_COMPUTES * 32768 * 32 * sizeof(_u8), 256);
      ccann::alloc_aligned((void **) &buf.aligned_pqtable_dist_scratch, 256 * MAX_PQ_CHUNKS * sizeof(float), 256);
      // [MAX_N_COMPUTES * MAX_DEGREE]
      ccann::alloc_aligned((void **) &buf.aligned_dist_scratch, MAX_N_COMPUTES * 512 * sizeof(float), 256);
      ccann::alloc_aligned((void **) &buf.aligned_query_T, this->aligned_dim * sizeof(T), 8 * sizeof(T));
      ccann::alloc_aligned((void **) &buf.update_buf, (2 * MAX_N_EDGES + 1) * SECTOR_LEN,
    ...
    QueryBuffer<T> *pop_query_buf(const T *query) {
      ...
      if (likely(query != nullptr)) {
        if (data_is_normalized) {
          ...
          float norm = ccann::compute_l2_norm(query, this->data_dim);
   ...
   public:
    ...
    std::vector<_u8> data;  // hold all compressed PQ vectors
    _u64 chunk_size;
    _u64 n_chunks;
    FixedChunkPQTable<T> pq_table;  // codebooks
   ...
   public:
    ...
    bool fast_delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool, int &to_evict, float tgt_nbr_dis);
    void slow_delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool, std::vector<uint32_t> &pruned_list,
    ...
    // Uses sequence number (id) to ensure ordering.
   ...
   public:
    ...
    // NOTE:
    // here in mem index
    // tag is id in disk index
    std::unique_ptr<Index<T, uint32_t>> mem_index_ = nullptr;
    ...
    uint32_t id2loc(uint32_t id) {
      ...
      if (id2loc_.find(id, loc)) {
        ...
        LOG(ERROR) << "id " << id << " not found in id2loc" << " in index " << _disk_index_file;
        crash();
        return kInvalidID;
      }
#endif
    ...
    // background insert_commit_thread for asynchronous PM id2loc table update.
    // Follows the same pattern as id2loc_func for reader-side consistency.
    void id2loc_insert_or_assign(uint32_t id, uint32_t loc) {
      id2loc_.insert_or_assign(id, loc);
      id2loc_pm_queue.push(std::make_pair(id, loc));
    }
    ...
    // returns <loc, need_read>
    std::vector<uint64_t> alloc_loc_compact(int n, std::vector<uint64_t> &hint_pages,
                                            std::set<uint64_t> &page_need_to_read) {
      std::lock_guard<std::mutex> lock(alloc_lock);
      std::vector<uint64_t> ret;
      int cur = 0;
      ...
      // lambda to remove allocated pages from hint_pages
      auto remove_hint_pages = [&](const std::vector<uint64_t> &hint_pages_to_remove) {
        std::set<uint64_t> to_remove_set(hint_pages_to_remove.begin(), hint_pages_to_remove.end());
        std::vector<uint64_t> new_hint_pages;
        for (auto &p : hint_pages) {
          if (to_remove_set.find(p) == to_remove_set.end()) {
            new_hint_pages.push_back(p);
          }
        }
        hint_pages = std::move(new_hint_pages);
      };

      std::vector<uint64_t> hint_pages_to_remove;
      // Rule #2: on-path partial-empty rule
      //          these pages should mostly hit cache.
      for (auto &p : hint_pages) {
#ifdef NO_POLLUTE_ORIGINAL
        ...
        page_layout.update_fn(p, [&](PageArr &v) {
          ...
          // this allocation allocates the entire page.
          if (cnt + (n - cur) >= nnodes_per_sector) {
            // remove the page from hint_pages to avoid repeated scans.
            hint_pages_to_remove.push_back(p);
          }
          // alloc them.
          for (uint32_t i = 0; i < nnodes_per_sector; ++i) {
            if (v[i] == kInvalidID) {
              ...
              cur++;
              if (cur == n) {
                remove_hint_pages(hint_pages_to_remove);
                return;
              }
            }
          }
    ...
    std::atomic<_u64> ckpt_id;

    uint32_t num_cpus;
    uint32_t peak_cpus;

    bool is_index_inserttable = false;
    ...
    uint32_t insert_phase(const T *point, const TagT &tag, uint32_t target_id, std::vector<Neighbor> &exp_node_info,
                          tsl::robin_map<uint32_t, T *> &coord_map, std::vector<uint32_t> &new_nhood,
                          std::vector<uint64_t> &page_ref, std::vector<uint8_t> &in_pq_coords);
   ...
   private:
    ...
    bool on_pm = false;  // data is on persistent memory

    // IO Timing
    struct ann_stats {
      std::atomic<uint64_t> insert_phase_time = 0;
      std::atomic<uint64_t> update_graph_time = 0;
      std::atomic<uint64_t> update_neighbor_time = 0;
      std::atomic<uint64_t> prune_neighbor_time = 0;
      std::atomic<uint64_t> update_metadata_time = 0;

      std::atomic<uint64_t> read_nodes_time = 0;

      std::atomic<uint64_t> search_phase_time = 0;
      std::atomic<uint64_t> update_PQ_vec_time = 0;
      std::atomic<uint64_t> prune_search_neighbors_time = 0;
      std::atomic<uint64_t> search_graph_time = 0;
      std::atomic<uint64_t> populate_pq_dists_time = 0;
      std::atomic<uint64_t> calc_best_node_time = 0;
      std::atomic<uint64_t> send_best_node_time = 0;
      std::atomic<uint64_t> do_read_best_node_time = 0;
      std::atomic<uint64_t> poll_all_time = 0;
      std::atomic<uint64_t> calc_exact_dist_time = 0;
      std::atomic<uint64_t> expand_neighbors_time = 0;

      std::atomic<uint64_t> merge_time = 0;
      std::atomic<uint64_t> journal_time = 0;

      // IO Stat
      std::atomic<uint64_t> calc_best_node_number = 0;
      std::atomic<uint64_t> send_best_node_number = 0;
      std::atomic<uint64_t> poll_hit_number = 0;
      std::atomic<uint64_t> poll_number = 0;
      std::atomic<uint64_t> ncalc_for_expanding_neighbors = 0;
      std::atomic<uint64_t> ncalc_for_useless_neighbors = 0;
      std::atomic<uint64_t> expand_loop_number = 0;

      // IO stat for para search
      std::atomic<uint64_t> para_busy_wait_number = 0;
      std::atomic<uint64_t> para_wait_for_flight_number = 0;
      std::atomic<uint64_t> para_useless_wait_for_flight_number = 0;
      std::atomic<uint64_t> para_direct_hit_number = 0;
      std::atomic<uint64_t> para_early_exit_number = 0;
      std::atomic<uint64_t> para_likely_early_exit_number = 0;
    };

    std::vector<ann_stats *> io_timing_stats;
  };
}  // namespace ccann
