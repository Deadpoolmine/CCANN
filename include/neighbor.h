...
namespace ccann {
  ...
  struct Neighbor {
    unsigned id;
    float distance;
    // whether on flight: false - on flight, true - not issue or finished in CCANN
    bool flag; 
    bool visited;
  ...
  // reinsert into retset for next exploration.
  // we always want to keep retset sorted.
  // insert in the ascending order, i.e., closest distance is in the front.
