...
struct FlushRequest {
  ...
  FlushRequest() : buf(nullptr), len(0) {
  }

  FlushRequest(void *buf, uint64_t len) : buf(buf), len(len) {
    // 
  }
};
...
namespace ccann {
  ...
  struct QueryBuffer {
    ...
    float *aligned_dist_scratch = nullptr;          // MUST BE AT LEAST ccann [MAX_N_COMPUTES * MAX_DEGREE], for exact dist.    
    _u8 *aligned_pq_coord_scratch = nullptr;  // MUST BE AT LEAST  [MAX_N_COMPUTES * N_CHUNKS * MAX_DEGREE], for neighbor PQ vectors.
    _u64 comp_idx = 0;
