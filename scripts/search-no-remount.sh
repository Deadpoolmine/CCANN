...
if [[ "${WORKLOAD}" == "bigann-small" ]]; then
    ...
    TRUTH_SET=/home/data/deadpool/ANN/SIFT-SMALL/data/siftsmall_groundtruth.bin
    ...
    TRUTH_SET=/home/data/deadpool/ANN/SIFT-100M/data/100M_gt.bin
    ...
    TRUTH_SET=/home/data/deadpool/ANN/DEEP-100M/data/100M_gt.bin
...
if (( PERF == 1 )); then    
    ...
    sudo perf record -g -F 5000 -o $PERF_DATA -- \
        $UTILS_PATH/search "$TYPE_PREFIX" $L_disk $SEARCH_THREADS $SEARCH_MODE $INDEX_PREFIX $QUERY_BIN $TRUTH_SET $RECALL $BEAM_WIDTH $SEARCH_BEAM_WIDTH $SEARCH_MEM_L ${SEARCH_L[@]}
