...
ANNS=("cc-ann" "cc-ann-no-async-insert" "odinann-improve" "odinann" "cc-ann-journal")
# ANNS=( "cc-ann-naive" "odinann-improve" "odinann")
WORKLOADS=("bigann-100m" "deep-100m")
# TODO: deep-100m insert study
    ...
    for media in "${MEDIAS[@]}"; do
        ...
        for ann in "${ANNS[@]}"; do
            ...
            for t in "${THREADS[@]}"; do
                ...
                OUTPUT=$(bash "$ABS_PATH"/../insert.sh PipeANN "$media" 0 "$t" "$WORKLOAD" $batch_size $n_batches "$search_mode")
