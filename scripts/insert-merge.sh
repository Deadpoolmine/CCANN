...
MERGE_INTERVAL=$9 # 10000 by default
...
if (( PERF == 1 )); then    
    ...
    sudo perf record -g -F 5000 -o $PERF_DATA -- \
        $UTILS_PATH/insert_merge uint8 $DATA_BIN $L_disk $VECS_PER_STEP $NUM_STEPS $INSERT_THREADS $INDEX_PREFIX $BEAM_WIDTH
    ...
    $UTILS_PATH/insert_merge $TYPE_PREFIX "$DATA_BIN" $L_disk "$VECS_PER_STEP" "$NUM_STEPS" "$INSERT_THREADS" "$INDEX_PREFIX" $BEAM_WIDTH "$MERGE_INTERVAL" "$SEARCH_MODE"
fi
