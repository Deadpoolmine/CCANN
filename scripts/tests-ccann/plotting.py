...
import os
import sys
os.chdir("/mnt/nvme2/CCANN")

# the process function for CCANN, DiskANN and Starling.
# Input: the raw log file
# Output: a dict with key: system + dataset + threads, value: list of latency, throughput and recall.
def process_ccann(filename):
    mode_sysname = ["DiskANN", "Starling", "CCANN", "Coro", "SPANN", "Vamana"]
    key = ""
    ret = {}
...
def figure11():
    raw_data = [process_ccann("./data/fig11_ccann.txt"), process_ccann("./data/fig11_diskann.txt"), process_ccann("./data/fig11_starling.txt"), process_spann("./data/fig11_spann.txt")]
...
def figure12():
    raw_data = [process_ccann("./data/fig12_ccann.txt"), process_ccann("./data/fig12_diskann.txt"), process_ccann("./data/fig12_starling.txt"), process_spann("./data/fig12_spann.txt")]
    plot_data = reorganize_data(raw_data)
...
def figure13():
    raw_data = [process_ccann("./data/fig13_ccann.txt"), process_ccann("./data/fig13_diskann.txt")]
    plot_data = reorganize_data(raw_data)
...
def figure14():
    raw_data = [process_ccann("./data/fig14_ccann.txt"), process_ccann("./data/fig14_diskann.txt")]
    plot_data = reorganize_data(raw_data)
    f = plot_tput_acc(plot_data, min_recall=0.8) # shrink min_recall to see the full plot.
    f.savefig("./figures/figure14.png", bbox_inches='tight')

def figure15():
    raw_data = [process_ccann("./data/fig11_ccann.txt"), process_ccann("./data/fig15_vamana.txt")]
    plot_data = reorganize_data(raw_data)
...
def figure17():
    ...
    raw_data_lat = [process_ccann("./data/fig17_static_lat.txt"), process_ccann("./data/fig11_ccann.txt")]
    data_lat = [{}, {}]
    for k, v in raw_data_lat[0].items():
        re_k = k.replace("CCANN", "Static")
        data_lat[0][re_k] = v
    for k, v in raw_data_lat[1].items():
        re_k = k.replace("CCANN", "Dynamic")
        data_lat[1][re_k] = v
    ...
    raw_data_tput = [process_ccann("./data/fig17_static_tput.txt"), process_ccann("./data/fig12_ccann.txt")]
    data_tput = [{}, {}]
...
def figure18():
    raw_data = [process_ccann("./data/fig11_ccann.txt"), process_ccann("./data/fig11_diskann.txt")]
    plot_data = reorganize_data(raw_data)
    ...
    def plot_recall_L(data, min_L, max_L):
        ...
        for i, dataset_name in enumerate(dataset_names):
            ...
            ccann_raw = methods_data.get('CCANN', [])
            diskann_raw = methods_data.get('DiskANN', [])

            # Filter data based on max_L and tuple length validity
            ccann_filtered = [item for item in ccann_raw if len(item) > max(x_index, y_index) and item[x_index] <= max_L and item[x_index] >= min_L]
            ...
            all_L_values = set(item[x_index] for item in ccann_filtered) | set(item[x_index] for item in diskann_filtered)
            sorted_L_values = sorted(list(all_L_values))

            ccann_dict = {item[x_index]: item[y_index] for item in ccann_filtered}
            diskann_dict = {item[x_index]: item[y_index] for item in diskann_filtered}

            ccann_y = [ccann_dict.get(L, 0) for L in sorted_L_values]
            ...
            rects1 = current_ax.bar(x_pos - width/2, ccann_y, width, label='CCANN', color='tab:blue')
