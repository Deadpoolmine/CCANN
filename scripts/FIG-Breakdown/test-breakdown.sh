...
for WORKLOAD in "${WORKLOADS[@]}"; do
    ...
    for media in "${MEDIAS[@]}"; do
        ...
        for ann in "${ANNS[@]}"; do
            if [[ "${ann}" == *"odinann"* ]]; then
                bash "$ABS_PATH"/../tools/setup-cc-ann.sh PipeANN "odinann" 1
                insert_t=$NUM_INSERT_THREADS
                search_t=$NUM_SEARCH_THREADS_ODINANN
            elif [[ "${ann}" == *"cc-ann"* ]]; then
                bash "$ABS_PATH"/../tools/setup-cc-ann.sh PipeANN "$ann" 1
                insert_t=$NUM_INSERT_THREADS
            ...
            bash "$ABS_PATH"/../overall.sh PipeANN "$media" 0 "$insert_t" "$search_t" $batch_size $n_batches "$WORKLOAD" "$search_mode" 1 |& tee "$output_path"
