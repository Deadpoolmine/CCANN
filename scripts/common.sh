...
function copy_file_multithreads() {
    local index_path="$1"
    ...
    echo "Finished copying files to $target_index."
    stop_http_server "$index_port"
}

function broadcast_to_pm() {
    local index_path="$1"
    local fs_name="$2"

    setup_pm_fs "$fs_name" "/mnt/pmem0"
    setup_pm_fs "$fs_name" "/mnt/pmem1"

    # check whether aria2c is installed
    if ! command -v aria2c &> /dev/null; then
        echo "aria2c could not be found, using rsync for migration ..."
        rsync -a --info=progress2 "$index_path/" "/mnt/pmem0/index/" &
        rsync -a --info=progress2 "$index_path/" "/mnt/pmem1/index/" &
        wait
    else
        echo "aria2c found, using aria2c for migration ..."
        ...
        index_port2=8083
        target_index="/mnt/pmem0/index"
        copy_file_multithreads "$index_path" "/mnt/pmem0/index" "$index_port1" &
        copy_file_multithreads "$index_path" "/mnt/pmem1/index" "$index_port2" &
...
function extract_breakdown() {
    path=$1
    item=$2

    grep "$item" "$path" | sed -E 's/.*: +([0-9]+).*/\1/' | awk '{sum+=$1} END {print sum}'
}